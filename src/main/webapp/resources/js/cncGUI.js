...
var gui = new function() {
				...
				.confirm(
						...
						function(confirmed) {
							if (confirmed) {
								var thisForm = $(gui.form);
								if (thisForm.attr("submit-url")) {
									gui.executeSubmission(
											thisForm.attr("submit-url"),
	...
	this.submissionHandler = function(clickEvent) {
		...
		if (gui.renderGrid) {
			...
			/*
			 * si no hay grilla es necesario se deben extraer los datos del
			 * formulario crudo y si es un draft, no hace falta validar caso
			 * contrario se debera validar todo el formulario antes de realizar
			 * el submit
			 */
		} else {
			if (draft == true) {
